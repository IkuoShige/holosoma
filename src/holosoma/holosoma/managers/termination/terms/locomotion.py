...
def dof_velocity_limit_exceeded(env, threshold_scale: float = 1.0, probability: float = 1.0) -> torch.Tensor:
    """Terminate when DOF velocity limits are exceeded."""
    delta = (torch.abs(env.simulator.dof_vel) - env.dof_vel_limits * threshold_scale).clip(min=0.0, max=1.0)
    violation = torch.sum(delta, dim=1) > 0.0
    return _apply_probability(violation, probability, env.device)


def torque_limit_exceeded(env, threshold_scale: float = 1.0, probability: float = 1.0) -> torch.Tensor:
    """Terminate when actuator torques exceed limits."""
    torques = env.action_manager.get_term("joint_control").torques
    delta = (torch.abs(torques) - env.torque_limits * threshold_scale).clip(min=0.0, max=1.0)
    violation = torch.sum(delta, dim=1) > 0.0
    return _apply_probability(violation, probability, env.device)
