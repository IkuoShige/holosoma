...
def create_envs(
    ...
    **kwargs: Any,
    ...
    if env_type in ["dmc", "mujoco", "humanoid_bench", "metaworld", "myosuite", "d4rl"]:
        ...
        from holosoma._vendored.flash_rl.envs.maniskill import make_maniskill_env
        ...
        from holosoma._vendored.flash_rl.envs.mujoco_playground import make_mujoco_playground_env
        ...
        from holosoma._vendored.flash_rl.envs.genesis import make_genesis_env
...
def create_vec_env(
    ...
    def make_one_env(
        ...
        **kwargs: Any,
        ...
        if env_type == "mujoco":
            from holosoma._vendored.flash_rl.envs.mujoco import make_mujoco_env

            env = make_mujoco_env(env_name, seed, **kwargs)
        elif env_type == "d4rl":
            from holosoma._vendored.flash_rl.envs.d4rl import make_d4rl_env

            env = make_d4rl_env(env_name, seed)
        elif env_type == "dmc":
            from holosoma._vendored.flash_rl.envs.dmc import make_dmc_env

            env = make_dmc_env(env_name, seed, **kwargs)
        elif env_type == "humanoid_bench":
            from holosoma._vendored.flash_rl.envs.humanoid_bench import make_humanoid_env

            env = make_humanoid_env(env_name, seed, **kwargs)
        elif env_type == "myosuite":
            from holosoma._vendored.flash_rl.envs.myosuite import make_myosuite_env

            env = make_myosuite_env(env_name, seed, **kwargs)
        elif env_type == "metaworld":
            from holosoma._vendored.flash_rl.envs.metaworld import make_metaworld_env
...
def create_dataset(env_type: str, env_name: str) -> list[dict[str, Any]]:
    if env_type == "d4rl":
        from holosoma._vendored.flash_rl.envs.d4rl import make_d4rl_dataset
...
def get_normalized_score(env_type: str, env_name: str, unnormalized_score: float) -> float:
    if env_type == "d4rl":
        from holosoma._vendored.flash_rl.envs.d4rl import get_d4rl_normalized_score
